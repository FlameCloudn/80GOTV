...
def run():
    if not Config.DEFAULT_ADMIN_PASSWORD:
        raise RuntimeError("未设置 ADMIN_PASSWORD，已停止初始化。请先在 .env 中设置管理员密码。")
    ...
    for name, short, desc in teams:
        c.execute(
            "INSERT INTO teams(name, short_name, description) VALUES(?,?,?)", (name, short, desc)
        )
    ...
    for tid in range(1, 9):
        for nick, real in nicks:
            c.execute(
                "INSERT INTO players(nickname, real_name, team_id, steam_id) VALUES(?,?,?,?)",
    ...
    for ev in events_data:
        c.execute(
            """INSERT INTO events(name, short_name, description, start_date, end_date, format, status)
                     VALUES(?,?,?,?,?,?,?)""",
            ev,
        )
    ...
    for m in matches_data:
        c.execute(
            """INSERT INTO matches(event_id, team1_id, team2_id, team1_score,
    ...
    for match in completed_matches:
        ...
        for team_id in [t1_id, t2_id]:
            ...
            for p in players:
                ...
                c.execute(
                    """INSERT INTO match_stats(match_id, player_id, team_id, kills,
    ...
    news_data = [
        ...
        (
            ...
            "数据看选手",
            "admin",
            (today - timedelta(hours=12)).isoformat(),
            19,
        ),
        (
    ...
    for n in news_data:
        c.execute(
            """INSERT INTO news(title, content, summary, author, publish_time, comment_count)
                     VALUES(?,?,?,?,?,?)""",
            n,
        )
    ...
    # ---- 管理员账号 ----
    admin = c.execute(
        "SELECT username FROM admins WHERE username=?", (Config.DEFAULT_ADMIN_USERNAME,)
    ).fetchone()
    if admin:
        print(f"✅ 管理员账号已就绪 ({Config.DEFAULT_ADMIN_USERNAME})")
    else:
        if not Config.DEFAULT_ADMIN_PASSWORD:
            conn.close()
            raise RuntimeError(
                "未设置 ADMIN_PASSWORD，已停止创建空密码管理员。请先在 .env 中设置管理员密码。"
            )
        c.execute(
            "INSERT INTO admins(username, password_hash) VALUES(?,?)",
            (Config.DEFAULT_ADMIN_USERNAME, generate_password_hash(Config.DEFAULT_ADMIN_PASSWORD)),
        )
        print(f"✅ 创建管理员账号 ({Config.DEFAULT_ADMIN_USERNAME})")
